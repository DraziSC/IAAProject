...
def update_world(game_state):
    ...
    if grid[pacman_pos[1]][pacman_pos[0]] == DOT:
        ...
        check_won(game_state)
        game_state['score'] += 5
        game_state['scared_ghosts_timestamp'] = time.time()#SCARED_GHOST_TIME*FPS
        for ghost in game_state['ghosts']:
            ghost['scared'] = True
...
def check_collisions(pacman, ghosts, game_state):
    for ghost in ghosts:
        if ghost['alive']:
            if manhattan_distance((ghost['x'], ghost['y']), (pacman['x'], pacman['y'])) < 1:
                if ghost['scared']:
                    ghost_eaten(ghost, game_state)
                else:
                    pacman_eaten(game_state)
...
def main(pacman_policy, ghost_policies, frightened_ghost_policies, map_file = "originalClassic.txt"):
    ...
    pacman = {'x':1, 'y':1, 'previous_direction': None, 'direction' : None , 'next_direction' : None, 'image':None, 'model':None, 'DOT_TRUE_PROB':(0.5,0.5), 'GHOST_TRUE_PROB':(0.5,0.5), 'SCARED_GHOST_TRUE_PROB':(0.5,0.5)}  # Default position if not found in map
    ghosts = []
    ...
    for y in range(grid_size[1]):
        for x in range(grid_size[0]):
            if grid[y][x] == PACMAN:
                ...
                ghosts.append( {'start_x':x, 'start_y':y, 'x':x, 'y':y, 'direction':None, 'alive':True, 'image':None, 'name':ghost_names[len(ghosts)], 'scared':False} )
                grid[y][x] = EMPTY
...
def set_pacman_direction(game_state,dir):
    ...
    if dir in get_valid_directions(pacman_pos, game_state['grid'], game_state['grid_size']):
